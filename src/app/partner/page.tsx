...
export default function PartnerPage() {
  ...
  const inp: React.CSSProperties = {
    width: "100%", background: "#0c0e16", border: "1px solid #1e2230",
    borderRadius: "0.625rem", color: "#fafafa", fontSize: "0.9375rem",
    padding: "0.75rem 0.875rem", outline: "none",
  };
  const lbl: React.CSSProperties = { display: "block", fontSize: "0.8125rem", fontWeight: 500, color: "#9aa0ad", marginBottom: "0.5rem" };
  const featured = games.slice(0, 5);

  if (sent) {
    return (
      <div style={{ paddingTop: "4rem", minHeight: "100vh", background: "#080a0f", display: "flex", alignItems: "center", justifyContent: "center", padding: "5rem 1.25rem" }}>
        <div style={{ textAlign: "center", maxWidth: "30rem" }}>
          <div style={{ width: "3.5rem", height: "3.5rem", borderRadius: "1rem", background: "rgba(245,166,35,0.12)", border: "1px solid rgba(245,166,35,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1.5rem" }}>
            <CheckCircle2 size={26} style={{ color: "#f5a623" }} />
          </div>
          <h1 style={{ fontSize: "1.75rem", fontWeight: 800, color: "#fafafa", marginBottom: "0.75rem" }}>Message sent</h1>
          <p style={{ fontSize: "1rem", color: "#9aa0ad", lineHeight: 1.7 }}>
  ...
  return (
    <div style={{ paddingTop: "4rem", minHeight: "100vh", background: "#080a0f" }}>
      <style>{`
      ...
      <div style={{ maxWidth: "75rem", margin: "0 auto", padding: "4rem 1.25rem 5rem" }}>
        <div className="partner-grid">
          ...
          {/* LEFT — pitch */}
          <div>
            <p style={{ fontSize: "0.6875rem", fontFamily: "monospace", color: "#f5a623", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: "1rem" }}>Partner with us</p>
            ...
            <p style={{ fontSize: "1.0625rem", color: "#9aa0ad", lineHeight: 1.75, marginBottom: "2.5rem", maxWidth: "30rem" }}>
              ...
              {benefits.map(b => (
                <div key={b.title} style={{ display: "flex", gap: "0.875rem", alignItems: "flex-start" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: "0.9375rem", color: "#71757f", lineHeight: 1.55 }}>{b.body}</div>
                  </div>
                </div>
              ))}
            </div>

            <p style={{ fontSize: "0.9375rem", color: "#71757f" }}>
              Prefer email? <a href="mailto:hello@kani.studio" style={{ color: "#f5a623", textDecoration: "none", fontWeight: 600 }}>hello@kani.studio</a>
            </p>
          </div>

          {/* RIGHT — form */}
          <div className="partner-form-card" style={{ background: "#0c0e16", border: "1px solid #161a26", borderRadius: "1.25rem" }}>
            {error && (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem", marginBottom: "1.25rem" }}>
              <div>
                <label style={lbl}>Name <span style={{ color: "#f5a623" }}>*</span></label>
                <input value={form.name} onChange={set("name")} placeholder="Your name" style={inp} />
              </div>
              <div>
                <label style={lbl}>Work email <span style={{ color: "#f5a623" }}>*</span></label>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem", marginBottom: "1.25rem" }}>
              <div>
                <label style={lbl}>Company <span style={{ color: "#f5a623" }}>*</span></label>
                <input value={form.company} onChange={set("company")} placeholder="Company / brand" style={inp} />
              </div>
              <div>
                <label style={lbl}>You are a... <span style={{ color: "#f5a623" }}>*</span></label>
                <select value={form.role} onChange={set("role")} style={{ ...inp, color: form.role ? "#fafafa" : "#5a5e6b" }}>
            ...
            <div style={{ marginBottom: "1.25rem" }}>
              <label style={lbl}>Games you&apos;re interested in <span style={{ color: "#5a5e6b" }}>(optional)</span></label>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                {featured.map(g => {
                  const active = selectedGames.includes(g.id);
                  return (
                    <button key={g.id} onClick={() => toggleGame(g.id)} type="button"
                      style={{ fontSize: "0.75rem", fontWeight: 600, fontFamily: "monospace", letterSpacing: "0.03em", padding: "0.5rem 0.875rem", borderRadius: "9999px", cursor: "pointer", textTransform: "uppercase", border: "1px solid", background: active ? "rgba(245,166,35,0.15)" : "#0c0e16", color: active ? "#f5a623" : "#9aa0ad", borderColor: active ? "rgba(245,166,35,0.4)" : "#1e2230" }}>
                      {g.title}
                    </button>
                  );
                })}
                <button onClick={() => setSelectedGames(selectedGames.length === featured.length ? [] : featured.map(g => g.id))} type="button"
                  style={{ fontSize: "0.75rem", fontWeight: 600, fontFamily: "monospace", padding: "0.5rem 0.875rem", borderRadius: "9999px", cursor: "pointer", border: "1px solid #1e2230", background: "#0c0e16", color: "#9aa0ad" }}>
                  Whole portfolio
                </button>
              </div>
            </div>
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              <label style={lbl}>Anything else we should know? <span style={{ color: "#5a5e6b" }}>(optional)</span></label>
            ...
            <button onClick={submit} disabled={loading}
              style={{ width: "100%", background: loading ? "#a8741a" : "#f5a623", color: "#09090b", fontWeight: 700, fontSize: "1rem", padding: "0.9375rem", borderRadius: "0.75rem", border: "none", cursor: loading ? "not-allowed" : "pointer" }}>
              {loading ? "Sending…" : "Send — we reply within 24h"}
            </button>
            <p style={{ textAlign: "center", fontSize: "0.8125rem", color: "#5a5e6b", marginTop: "1rem" }}>
