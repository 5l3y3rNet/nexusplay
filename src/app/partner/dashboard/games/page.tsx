...
export default function PartnerGamesPage() {
  ...
  if (authLoading) return <div style={{ height: "100vh", background: "#080a0f", display: "flex", alignItems: "center", justifyContent: "center", color: "#71757f" }}>Loading…</div>;
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <header style={{ height: "4rem", borderBottom: "1px solid #1c1f2b", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 1.5rem", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
            ...
            <span style={{ fontSize: "0.75rem", fontFamily: "monospace", color: "#5a5e6b", background: "#0c0e16", border: "1px solid #161a26", padding: "0.125rem 0.5rem", borderRadius: "0.25rem" }}>{games.length}</span>
          </div>
        ...
        <main style={{ flex: 1, overflowY: "auto", padding: "1.5rem" }}>
          {loading ? <div style={{ padding: "3rem", textAlign: "center", color: "#5a5e6b" }}>Loading…</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "1rem" }}>
              {games.map(game => (
                <div key={game.id} style={{ background: "#0c0e16", border: "1px solid #161a26", borderRadius: "0.75rem", overflow: "hidden" }}>
                  <div style={{ height: "9rem", background: "#080a0f", position: "relative" }}>
                    <img src={game.thumbnail} alt={game.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    <span style={{ position: "absolute", top: "0.5rem", left: "0.5rem", fontSize: "0.625rem", background: "rgba(12,14,22,0.9)", color: "#9aa0ad", border: "1px solid #1e2230", padding: "0.125rem 0.5rem", borderRadius: "0.25rem", fontFamily: "monospace" }}>{game.category}</span>
                  </div>
