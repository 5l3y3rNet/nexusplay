    ...
    <div style={{ display: "flex", height: "100vh", background: "#070a14", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <main style={{ flex: 1, overflowY: "auto", padding: "1.5rem" }}>
          <div style={{ display: "flex", gap: "0.25rem", marginBottom: "1.25rem", background: "#0f1424", border: "1px solid #1e2740", borderRadius: "0.5rem", padding: "0.25rem", width: "fit-content" }}>
            {["all", "new", "read", "replied"].map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: "0.375rem 0.875rem", fontSize: "0.8125rem", borderRadius: "0.375rem", border: "none", cursor: "pointer", textTransform: "capitalize", background: filter === f ? "#1e2740" : "transparent", color: filter === f ? "#fafafa" : "#71757f" }}>{f}</button>
            ))}
          </div>
          ...
          {loading ? <div style={{ padding: "3rem", textAlign: "center", color: "#5a5e6b" }}>Loading…</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
              {filtered.map(i => (
                <div key={i.id} style={{ background: "#0f1424", border: `1px solid ${i.status === "new" ? "rgba(255,255,255,0.18)" : "#1e2740"}`, borderRadius: "0.75rem", padding: "1.25rem" }}>
                  ...
                  {i.games_interested?.length > 0 && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.375rem", marginBottom: "0.75rem" }}>
                      {i.games_interested.map(g => <span key={g} style={{ fontSize: "0.6875rem", fontFamily: "monospace", background: "#1e2740", color: "#9aa0ad", padding: "0.125rem 0.5rem", borderRadius: "0.25rem" }}>{g}</span>)}
                    </div>
                  )}

                  {i.message && <p style={{ fontSize: "0.875rem", color: "#a8adb8", lineHeight: 1.6, marginBottom: "1rem", padding: "0.75rem", background: "#070a14", borderRadius: "0.5rem", border: "1px solid #1e2740" }}>{i.message}</p>}

                  <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                    {i.status !== "read" && <button onClick={() => setStatus(i.id, "read")} style={{ fontSize: "0.75rem", background: "#1e2740", color: "#c4c7cf", border: "1px solid #1e2740", padding: "0.375rem 0.75rem", borderRadius: "0.375rem", cursor: "pointer" }}>Mark read</button>}
