        ...
        <main style={{ flex: 1, overflowY: "auto", padding: "1.5rem" }}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem", marginBottom: "1.25rem" }}>
            <div style={{ position: "relative", flex: "1 1 200px", minWidth: "200px", maxWidth: "20rem" }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search partners..." style={{ width: "100%", background: "#131a2e", border: "1px solid #1e2740", borderRadius: "0.5rem", color: "#fafafa", fontSize: "0.875rem", padding: "0.5rem 0.75rem 0.5rem 2.25rem", outline: "none" }} />
            </div>
            <div style={{ display: "flex", gap: "0.25rem", background: "#131a2e", border: "1px solid #1e2740", borderRadius: "0.5rem", padding: "0.25rem", overflowX: "auto" }}>
          ...
          <div style={{ background: "#0f1424", border: "1px solid #1e2740", borderRadius: "0.75rem", overflow: "hidden" }}>
            {loading ? <div style={{ padding: "3rem", textAlign: "center", color: "#52525b" }}>Loading…</div>
              ...
              : filtered.map(p => (
              <div key={p.id} style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "1rem 1.25rem", borderBottom: "1px solid #1e2740", flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: "0.375rem" }}>
                  ...
                  {p.status === "active" && <button onClick={() => setStatus(p.id, "suspended")} style={{ fontSize: "0.75rem", background: "#131a2e", color: "#71717a", border: "1px solid #1e2740", padding: "0.375rem 0.75rem", borderRadius: "0.375rem", cursor: "pointer" }}>Suspend</button>}
                  {p.status === "suspended" && <button onClick={() => setStatus(p.id, "active")} style={{ fontSize: "0.75rem", background: "#131a2e", color: "#71717a", border: "1px solid #1e2740", padding: "0.375rem 0.75rem", borderRadius: "0.375rem", cursor: "pointer" }}>Reinstate</button>}
                </div>
              </div>
            ))}
          </div>
